...
async function importHierarchyStrict() {
  ...
  const getUnallocatedSite = async () => {
    ...
    if (!site) site = await siteRepo.save(siteRepo.create({ name: 'UNALLOCATED' }));
    return site;
  };
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      if (!category) await categoryRepo.save(categoryRepo.create({ name: catName }));
      ...
      let bldgName = row['BLDG. LOCATION']?.replace(/\t/g, '')?.replace(/"/g, '')?.trim() || 'N/A';
      if (bldgName.toUpperCase() === 'N/A' || bldgName === '') bldgName = 'UNALLOCATED';

      let floorName = row['FLOOR']?.replace(/\t/g, '')?.replace(/"/g, '')?.trim() || 'N/A';
      if (floorName.toUpperCase() === 'N/A' || floorName === '') floorName = 'UNALLOCATED';

      let divName = row['DIVISION']?.replace(/\t/g, '')?.replace(/"/g, '')?.trim() || 'N/A';
      if (divName.toUpperCase() === 'N/A' || divName === '') divName = 'UNALLOCATED';

      let deptName = row['DEPARTMENT']?.replace(/\t/g, '')?.replace(/"/g, '')?.trim() || 'N/A';
      if (deptName.toUpperCase() === 'N/A' || deptName === '') deptName = 'UNALLOCATED';
      ...
      if (!unit) await unitRepo.save(unitRepo.create({ name: unitName, departmentId: department }));
