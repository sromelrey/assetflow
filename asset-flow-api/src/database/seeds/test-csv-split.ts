...
function analyzeCSV() {
    ...
    const commaSplit = firstLine.split(',').map(h => h.replace(/^"|"$/g, '').trim());
    console.log('First 10 headers if comma-split:', commaSplit.slice(0, 10));

    console.log('\n--- TEST 4: Header Mapping Check (TAB) ---');
    const tabSplit = firstLine.split('\t').map(h => h.replace(/^"|"$/g, '').trim());
    ...
    if (secondLine) {
        const dataSplit = secondLine.split(',').map(v => v.replace(/^"|"$/g, '').trim());
        console.log('First row value for category if comma-split (index 8?):', dataSplit[8] || 'N/A');
